...
- 当前本地入口只监听 `127.0.0.1:8080`；Docker Desktop 下 Core 通常只能看到 nginx 的 bridge 地址。需要 `rip` 的关注/评论/转发必须显式配置 `WEIBO_PUBLIC_RIP` 为 Worker 的真实公共出口；缺失或非公共地址时继续 fail closed。
- 若由反向代理提供客户端公共地址，必须把该代理的实际 Docker/生产网段显式加入 `WEIBO_TRUSTED_PROXY_CIDRS`，并确保 Core 不能被绕过代理直连。Core 只接受这些 peer 覆盖的 `X-Real-IP`，始终忽略 `X-Forwarded-For`；不得使用宽泛私网默认值或信任客户端自填请求头。
