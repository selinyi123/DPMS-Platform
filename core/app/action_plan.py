...
# Extract complete handle tokens rather than accepting substring prefixes:
# reviewed ``@alice`` must not be considered present in ``@alice2``.
...
def validate_weibo_preflight_unique_handle_limit(
    action_payloads: Mapping[str, Mapping[str, Any]],
    content_requirements: Mapping[str, Any],
    source_content_requirements: Mapping[str, Any],
) -> None:
    """Bound the total identities expanded by a single Weibo preflight.

    Per-field limits do not constrain the union of comment, repost, source and
    follow handles. Merge every reviewed handle location by the same
    NFKC/casefold identity used for friend binding before enforcing the
    whole-plan cap.
    """

    handles: list[str] = []

    def extend_handles(values: Any) -> None:
        if not isinstance(values, list):
            return
        handles.extend(
            value
            for value in values
            if isinstance(value, str) and HANDLE_PATTERN.fullmatch(value)
        )
    ...
    for requirements in (
        ...
        if not isinstance(requirements, Mapping):
            continue
        extend_handles(requirements.get("follow_targets"))
        for action in CONTENT_REQUIREMENT_ACTIONS:
            action_requirements = requirements.get(action)
            if isinstance(action_requirements, Mapping):
                extend_handles(action_requirements.get("mentions"))

    followed_payload = action_payloads.get("followed")
    if isinstance(followed_payload, Mapping):
        target_handle = followed_payload.get("target_handle")
        if isinstance(target_handle, str) and HANDLE_PATTERN.fullmatch(
            target_handle
        ):
    ...
    for action in CONTENT_REQUIREMENT_ACTIONS:
        ...
        if isinstance(payload, Mapping):
            extend_handles(payload.get("mentions"))

    if len(set(_mention_identity_keys(handles))) > WEIBO_PREFLIGHT_UNIQUE_HANDLE_LIMIT:
        raise ActionPlanV2Error(
...
def validate_action_plan_v2(
    ...
    *,
    ...
    if raw_source_content_requirements is None:
        # Plans created before action-scoped friend constraints did not retain
        # the pre-binding source tokens. They remain compatible only when no
        # friend identities were added by an operator. A constrained plan
        # without this field cannot prove which @handles are friends.
