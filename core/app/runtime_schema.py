"""Legacy idempotent schema bootstrap used only by the migration command."""
...
async def ensure_consistency_schema():
    ...
    await database.execute(
        """CREATE TABLE IF NOT EXISTS outbox_events (
          ...
          INDEX idx_outbox_stream_status_id (stream_key, status, id)
        ) ENGINE=InnoDB"""
    )
    await ensure_column(
        "outbox_events",
